...
registerSearchLogs(server, sessionManager);
registerExtractCrashContext(server, sessionManager);
registerGetRunSummary(server, sessionManager);

const transport = new StdioServerTransport();
await server.connect(transport);
rootLog.info("server ready", { version: VERSION, transport: "stdio", tools: 15 });
