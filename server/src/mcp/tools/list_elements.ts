...
const description = [
  ...
  "Args: `runId`; optional `label` recorded in the `list_elements` event; optional `filter` (`{clickableOnly?, classContains?, textContains?, contentDescContains?, resourceIdContains?, inViewport?}`) to narrow at server-side — fields compose as AND, substring filters are case-insensitive, `resourceIdContains` is useful for bottom-nav ids, `inViewport` keeps only elements with at least one pixel inside `[0,w)×[0,h)`; optional `limit` (1-500, default 100) trims the post-filter list.",
