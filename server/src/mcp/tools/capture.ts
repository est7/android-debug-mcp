...
const description = [
  ...
  "Errors: `no_active_session` for an unknown runId; `device_disconnected` when the device has dropped; `adb_not_found` when the adb binary is missing; `adb_command_failed` when a `screencap` / `uiautomator` adb command fails. A failed ui_dump yields `uiDumpPath:null` (not an error); a screenshot that returns no PNG throws.",
].join("\n");
