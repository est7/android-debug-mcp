...
const description = [
  ...
  "Errors: `no_active_session` for an unknown runId; `device_disconnected` when the session's device has dropped; `adb_not_found` when the adb binary is missing; `adb_command_failed` when an adb command fails. Launch failures are reported in `detail` with `launched:false` rather than thrown.",
].join("\n");
