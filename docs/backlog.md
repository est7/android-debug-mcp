...
### v2-F · Element-based interaction(mobile-mcp 评估)

- **Motivation**:v1 的 `tap` / `swipe` 是坐标级,人/agent 都要先知道坐标。Element-based("点击文本为 Login 的按钮")更稳、更适合自动化复现。[`mobile-next/mobile-mcp`](https://github.com/mobile-next/mobile-mcp) 是相关开源 MCP。Q1 决策(见 [`decision-amendments.md` § B](./decision-amendments.md#b--interaction-实现路径明确q1-澄清))把它推到 v2。
- **触发条件**:满足任一即可启动:
  1. v1 实测里出现 ≥3 次"坐标在不同设备上漂移导致复现失败"的真实案例
  2. v2-B(auto-replay)启动时,必须有 element-based 作为稳定 hook
- **候选方案对比**(评估时再细化):
  1. **并列接入**(README 建议方案,无工程量):用户在 agent host 配两个 MCP,我们 + mobile-mcp 各管一摊;agent 自己决定哪个 tool 用。代价:mobile-mcp 的操作不进我们的 `events.jsonl`,session 证据链断。
  2. **嵌入式包装**:我们的 server 内 spawn 一个 mobile-mcp 子进程,把它的 tool 通过我们的接口转发,操作完再写 events.jsonl。代价:协议桥接复杂、外部 dep。
- **未决问题**:
  - 是否值得为了 session-aware 包一层?还是放弃 session-aware 接受并列方案?
  - mobile-mcp 在 v2 启动时的实现是否稳定到可绑定(项目活跃度、API 兼容性)
  - 我们自己加一个 `tap_element_by_text(runId, text)` 是否比接入 mobile-mcp 更省事(只需 uiautomator XML 找 node + 取 bounds 中心点)
