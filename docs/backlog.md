...
- **v2-G.1 Block B** —— `previewForAgent` 单条 record 截断(token 暴击,Poppo lang.json 实测单条 ~155k tokens)→ 见 § v2-G 条目下"Agent-facing per-record preview / truncation"
- **v2-F.3** —— `list_elements` filter + pagination(token 5×↓,v0.5.0 annotate 把 list 塞进 capture 后价值翻倍)→ 见下方独立 entry
...
### v2-F.3 · `list_elements` filter + pagination — **v0.6.0 Tier 1**

- **Motivation**:`list_elements` 现在一屏返 60-80 元素,80% noise;v0.5.0 annotate
  把同列表塞进 `capture` 响应 → 同样问题搬到 capture 上,**单次 token cost 翻倍价值**。
  实测一屏 Element 数组 ~15-40 KB(取决于 element 数量 × 文本字段),全送 agent
  context 一次扣 4-10k tokens。
- **新增 input fields**(向后兼容,全 optional):
  - `clickableOnly?: boolean` —— 只要 `clickable:true` 的
  - `classContains?: string` —— `class` 子串过滤(`"Button"` / `"Edit"` etc)
  - `inViewport?: boolean` —— 过滤 bounds 完全在屏外的
  - `limit?: number`(默认 unlimited,加 cap 时定 100 之类)
  - `cursor?: string` —— pagination,与 `limit` 一起
- **同步加到 capture 的 `annotateElements`** —— annotate 端共用 filter 形态。
  设计时 lock 一处 filter schema,两 tool 引用同一 zod object。
- **触发条件**:**v0.6.0 必做**(本 sprint Tier 1)
- **未决问题**:
  - filter 字段命名("classContains" vs "kindLike" vs ...)
  - pagination cursor 形态(offset / sortKey / opaque base64,与 evidence cursor 对齐?)
  - `inViewport` 是 viewport 完全 ⊂ 元素 bounds 还是相交(`isVisible: boolean` 区分?)
  - 与 v2-F.2 (c) `annotateElementIds` 关系:filter 是"产 list 端裁",annotateElementIds
    是"产 list 后只画/只返子集"。两者并存还是合并?
...
- **关键 known 技术债**(留至下月触发):
  ...
  - **Agent-facing per-record preview / truncation**(v2-G.1,2026-05-26 audit findings 触发)—— 当前 `search_evidence` / `extract_evidence_context` 返回 record 时 `body.text` / `decoded` 全文落 agent context(实测 Poppo lang.json 单条 ~622 KB ≈ 155k tokens)。v0.4.0 Block A 已经强制 narrowing filter 减少调用频次,但单条命中仍可能撑爆窗口。设计方向:`EvidenceSource` 加 `previewForAgent(record) → {preview, truncated, fullSizeBytes}`,read tools 默认走 preview 模式,新加 `fullRecords: true` 显式参数让 agent 知道要付代价。跨 EvidenceSource interface + 所有调用路径,需 codex grill + 多轮 review。触发条件:agent 在生产场景出现 context-cap 报告或我们准备接第二个 source。
  - **Block A 软 bypass:`tsMsRange:{from:0}`**(v2-G.1 sub-item,codex v0.4.0 audit 非阻塞 ACK)—— 现 `validateNarrowingFilter` 只看 `tsMsRange !== undefined`,所以 agent 打 `tsMsRange:{from:0}` 就过闸门(bindSession R1 clamp 到 `sessionStartMs` 后等同"全 session 流量")。Codex 判定:这是"防 accidental 裸 fetch-all"的设计选择,不是"防 adversarial 构造";agent 至少要主动打字才绕过。彻底堵需要 `from` 和 `to` 都强制 + 上限 window-size cap(e.g. ≤ 5 min)。建议**与 v2-G.1 Block B 一起做**:Block B 落 `previewForAgent` 时一并把窗口语义收紧(`tsMsRange` 必须 bounded;无界查询走 cursor + filter 即可)。
