...
### v2-G · Project profile + EvidenceSource adapter layer ⭐ NEXT MILESTONE

> **2026-05-25 grill 锁定 Q1–Q12,MVP only-evidence 范围 + 跳过 SourceProfile 抽离**。本条目是 design seed,impl 启动后 promote 到 `docs/v2/profile-and-evidence.md` design lock + `docs/v2/v2-g-implementation-plan.md`。

- **Motivation**:v2-A 源码映射链 (`source/recipe.ts` BaseBindingActivity / ViewBinding 正则) 是 Poppo-baked;v2-F.0 上 3 条 lock-level amendments 让人感觉 MCP 在迁就 Poppo。**直接 driver**:Poppo 端 `CustomHttpLoggingInterceptor` 已写完(`rtcrequestlibrary`),设备上落 `http_*.jsonl`(schema 契约见 [`submodulepoppo/docs/projects/http-log-jsonl-schema.md`](../../submodulepoppo/docs/projects/http-log-jsonl-schema.md) rev4);若不先抽 adapter 层,接入 HTTP log 就是 v2-A 链 T 二次重蹈 —— 这次更严重(Poppo-only evidence source,非通用 Android)。
- **触发条件**:已成立(`HEAD = cdf6cb6` after v2-F.0 close sweep,producer 已 land,blocker 全无)。
- **MVP 范围(2026-05-25 grill Q1)** —— **(b) only-evidence**:
  - **做**:`EvidenceProfile` + `EvidenceSource` 接口 + 通用 `search_evidence` / `extract_evidence_context` 工具 + Poppo HTTP adapter 作为第一个 reference 实现
  - **不做**:`SourceProfile` 抽离(v2-A `source/recipe.ts` Poppo-bake **暂留**)—— 触发条件 = **下个月接入其他项目(2026-06 月内)**;到时候真有 *第二个 app* 的 data point,设计 abstraction 才不会成单 app 经验下的 premature abstraction(YAGNI)
- **关键设计决策**(grill 全部 nod-locked,2026-05-25):
  - **Q2 profile 选择**:per-project `<projectRoot>/.android-debug-mcp/profile.json`,start_session 时 MCP 读;vanilla 项目零文件
  - **Q4 search_evidence shape**:**discriminated-union by source** —— `z.object({ runId, query: z.discriminatedUnion("source", [...]), limit, cursor }).strict()`;每个 source 自定义结构化字段(`pathPrefix` / `methodIn` / `outcome` / `excludeHeartbeat` / `tsMsRange` 等)
  - **Q5+ 时间窗驱动 lazy pull**:`sessionStartMs` 作左边界 + filename `<localDate>` 做 file selection + 1 天 buffer 兜跨夜;active file mtime cache(`runDir/evidence/<source>/.mtime-cache.json`)+ stop_session force seal 拉
  - **Q5+ 存储**:`runDir/evidence/<source>/http_<date>_<idx>.jsonl`
  - **Q5+ deviceTimezone**:`adb shell getprop persist.sys.timezone` 探测,落 `metadata.device.timezone`
  - **Q6 redaction**:`collect_bundle` 出口 redact(header `Authorization`/`Cookie`/`Set-Cookie`/`Set-Cookie2`/`Proxy-Authorization` value + query `_sign`/`_random` value + URL reconstruct),其他全 raw;policy hardcoded in bundle module(future amendment 下放 profile —— v2-G.1 candidate)
  - **Q7 工具数**:21 → 23,加 `search_evidence` + `extract_evidence_context` (后者镜像 v1 `extract_crash_context` 体例,保留对称感 + future server-side discovery 扩展空间)
  - **Q8 extract_evidence_context 输入**:`markerIsoTs: string`(agent 从 events.jsonl ts 直接拷)+ `beforeMs/afterMs`(默 5000/5000,上限 60_000)+ 复用 `search_evidence` 的 discriminated-union query(减 `tsMsRange`)
  - **Q9 持久化**:events.jsonl 只在 *真实拉* 发生时写 `{type:"evidence_pulled", source, trigger:"lazy"|"seal", files, recordCountDelta?, ts}`;commands.jsonl 走 aggregate `{tool:"search_evidence", statsRun, pullsTriggered, pulledFiles, ts}` capture-mirror 体例;summary.md `describeEvent` 加 case
  - **Q10 profile.json schema**:`{ name: string, version: 1 }` minimal,future field 走 `version: 2` SemVer bump;`.strict()` 拒 unknown field
  - **Q11 vanilla 项目行为**:`search_evidence` / `extract_evidence_context` **永远注册** (tool inventory 23 固定);无 profile / source 不可达 → **soft-empty + warnings**(`{records: [], warnings: ["session has no profile loaded; source 'poppo_http' has no provider"]}`);hard error 仅 `profile_malformed` / `profile_unknown`(start_session 路径,typed catalog)
  - **Q12 impl plan**:5 phase,串行,cadence A 2 次 audit(phase 3 后 contract lock + phase 5 后 final);估时 2-2.5 周(producer 已 land,纯 MCP reader)
- **关键 known 技术债**(✱ 必须留:下月触发):
  - **v2-A `source/recipe.ts` Poppo-bake 未抽离**:Phase 1 / v2-A 链 T 的 `BaseBindingActivity<XxxBinding>` 正则 / `binding.<camelEntry>` 代码引用模式 / layout → BindingClass 命名约定 全部 hardcoded。**触发条件:接入下个月任意非 Poppo Android 项目时** —— 触发后启动 v2-H(或叫 "SourceProfile extraction"),把现 `resolveCandidates` 行为 wrap 成 `poppo-vone` profile 的 `SourceResolver`,核心退到 vanilla(无 profile 时 confidence cap 到 low/medium honest 降级)。codex 估 6 phase 的原方案的 phase ②;v2-G MVP 跳过它就是为了延迟决策、等真实第二 app 数据进来再 abstract
  - **Q6 redaction policy profile-owned**(v2-G.1 candidate):MVP hardcoded list in `collect_bundle` 模块。**触发条件**:接入第二 source(可能就是 v2-H 时同时接入)且其敏感字段名 ≠ Poppo 的,redaction list 必须 per-source declare
- **acceptance scope**:8 真机 scenario(profile 加载 happy / profile 损坏 / search happy / extract around marker / lazy mtime hit / stop seal + bundle redact verify / release build 无 evidence 目录 soft 返 / vanilla 项目工具可见+软返)+ v2-A 70+ vitest 全绿 + 几条 v2-A tap_node 真机 sanity(不重跑 5 scenario)
- **release 节奏**:hold 0.4.0 tag,phase 5 末统一 cut `v2-F.0 + v2-G + Poppo HTTP adapter`(总 23 tool / 3 amendments / 4 lock docs)
- **依赖前置**:✅ Poppo `CustomHttpLoggingInterceptor` 已 land(`rtcrequestlibrary`)+ 设备有 `http_*.jsonl`;✅ v2-F.0 close sweep land(本 commit 周期)+ codex 双签;✅ schema doc P1 rev4 措辞 fix(本 grill 期 user 自找 — request body 是明文,`text` 字段说明已修订 in submodulepoppo 仓 unstaged)
