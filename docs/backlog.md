...
### 1.1-D · `resolveRunDir` 跨 runRoot 找不到(v0.4.0 acceptance 实测)

- **Motivation**:`server/src/store/locate.ts:27` `resolveRunDir` 先查 active sessions,再扫**当前** `resolveRunRoot()` 返回的目录。当 session 创建时用了 per-project `projectRoot`(走 explicit runRoot)、stop 之后再调 `collect_bundle` / `get_run_summary` / `extract_crash_context` / `search_logs`,no-arg `resolveRunRoot()` 走的是 env/cwd-git/fallback 链,**不会**复用 start 时的 explicit runRoot。文件源代码注释自承"a run created under a different runRoot is not found, which is acceptable for v1"。v0.4.0 acceptance S6 实测撞到 2 次;workaround 是"在 session 仍 active 时调 collect_bundle",但破坏了 test plan 的"stop → collect"自然顺序。
- **触发条件**:出现下面任一情况 → 启动:
  1. 用户(或 agent)在生产 debug 里报"过去某 run 找不到了"
  2. v0.5+ 准备启用多设备并发 session(v2-C)— 多 session 必然多 runRoot 形态,该限制会变常态痛点
- **方案候选**:
  - **A**:scanForRun 改成"先查 active sessions → 再查当前 runRoot → 再扫 `~/.android-debug-mcp/known-runRoots` 列表(每次 start_session 写一笔)"。让 server 自己记忆遇到过的 runRoot。
  - **B**:每个 run 在 `~/.android-debug-mcp/run-index/<runId>` 写一个 symlink 指回真 runDir;`resolveRunDir` 在 active miss 后查这个索引。比 A 干净(单一 source of truth)。
  - **C**:tool input 加 optional `projectRoot`,让调用方主动给出来。最低工作量但把责任推给 agent。
- **未决问题**:跨 host 共享 runRoot(用户 A 在 mac 起的 run,用户 B 在 mac 调 get_run_summary)是否要支持 → 直接 punt 到 v2-D(streamable HTTP)。
