...
- ✅ **v2-F.2** —— annotate iteration:badge collision (5 候选 + clamped) +
  ASCII text labels (`Button:1` from class FQCN) + `annotationIds` subset
  filter — **closed in v0.5.4**(真机 111-element home screen 7/7 acceptance pass;
  注:v2-F.2 重命名 `annotateElementIds` → `annotationIds` 避免 revive
  v2-F.1 lock 显式 anti-cache 的 `elementId` 词汇)
- ✅ **v2-G.2** —— `search_logs` aggregation(`count:true + groupBy + top`,
  no `errorType` until a stable classifier exists)— **implemented, pending audit/cut**
- ✅ **v2-A.1** —— `map_ui_node_to_source` tiered confidence filter
  (`minConfidence:"medium"|"high" + top`)— **implemented, pending audit/cut**
...
- ✅ **1.1-F · `getLocksRoot` test hermeticity env override**— **implemented, pending audit/cut**
...
### 1.1-F · `getLocksRoot` test hermeticity env override(parallel 1.1-D fix)— ✅ **implemented, pending audit/cut**

- **Resolution**:`getLocksRoot()` 支持 `ANDROID_DEBUG_MCP_LOCKS_ROOT`;Vitest
  setup 注入 per-process tmp dir,对齐 `ANDROID_DEBUG_MCP_INDEX_ROOT`。Production
  无 env 时仍落 `~/.android-debug-mcp/locks`。
- **验证**:`server/tests/store/paths.test.ts` 覆盖 env override 与 fallback;lock 测试
  通过 setup 自动 hermetic。
- **未决问题**:无。
...
- **Motivation**:v2-A v1 现在把全部 candidate 一股脑返,agent 自己挑;低置信度
  mapping 的 candidate 干扰 agent 决策。Server 端按整体 confidence verdict
  gate candidate list → agent context 干净,误判率 ↓。命中 ✅✅。
- **新增 input fields**:
  - `minConfidence?: "medium" | "high"`(默认不过滤,保 v2-A v1 行为)
  - `top?: number`(返确定性排序后的前 N 个 candidate)
- **触发条件**:**v0.6.0 Tier 2**(视余量)。即便 v2-A 仍在 Phase 1 View,filter
  已可用 —— 不依赖 Phase 2。
- **未决问题**:是否同步加进 `tap_node`(它内部也调 source mapping)的响应?
- **风险**:agent 习惯了 "candidate 列表很短" 后,真有 corner case 命中所有低置信度
  的时候反而拿不到提示 —— 当前倾向软返回 `candidates: []` +
  `warnings:["confidence_below_min"]`,不硬错、不 silent fallback。
...
### v2-F.2 · annotate iteration(badge collision + text labels + subset filter)— ✅ **closed in v0.5.4**

- **Resolution**:三 sub-features 单 sprint 单 cut。
  - (a) **Badge collision** — `placeBadgeWithCollision()` 5 候选(inside-TL/TR/BL/BR/
    outside-TL)first-non-overlap + 共享 `clampBadgeRect()` helper(半开矩形,与
    `hit_test.ts` / `element_filter.ts` 一致);全 5 候选撞则降级 outside-TL clamped。
  - (b) **ASCII labels** — `classShortLabel()` 取 FQCN 末段截 ≤ 8 chars
    (`android.widget.Button` → `Button` / `EditText` / `ImageBut` / `Recycler`);
    63-glyph 5×7 bitmap atlas(A-Z + a-z + 0-9 + `:`,`number[]` 行,~1.7 KB
    pure data,bun --compile 友好);badge 内容 = `<label>:<digits>` 横排
    (label scale=2 atlas + 7-segment digit,vertical center 对齐);**未做 i18n**
    (future v2-F.4 候选,触发条件 = popposhell Compose 上线 + 真出现中文 label 需求)。
  - (c) **`annotationIds?: number[]`** — post-filter+truncate ordinal selector
    (1-based 对应 trimmed 集合 1..N);dedup 静默 + ascending 输出 + out-of-range
    `query_malformed`;`subsetRequested` / `subsetApplied` 字段 surface 在
    `annotation` 块 + audit row;`annotationId` **不重编号**(保持 trimmed
    ordinal,F2-Q10)。**改名注解**:原 backlog 叫 `annotateElementIds`,v2-F.2
    Round 1 codex grill 抓为"revive v2-F.1 anti-cache 的 elementId 词汇",
    最终落 `annotationIds` 与现 `annotationId` 单数同源;stale-ordinal freshness
    模型显式写入 lock + tool description(F2-Q7b)。
- **Lock + provenance**:[`v2/annotate.md`](./v2/annotate.md) § Amendments
  § "2026-05-28 · v2-F.2"。Codex 2 rounds STOP fold-in(rename annotationIds +
  stale-ordinal lock / clampBadgeRect helper + 半开矩形 / atlas type bigint→number /
  63 glyph count / `.width` field-name drift)。Round 2 GO。
- **真机 acceptance**:device 951a20a2 home screen,111 elements,7/7 PASS
  (atlas inventory + classShortLabel × 8 cases + collision rotates 4 corners +
  annotatePng 8 labels + annotationIds subset [2,5,7] + 决定论 + 半开 boundary)。
- **Commit chain**:`<phase-1>` + v0.5.4 cut。
...
### v2-G.2 · `search_logs` aggregation — ✅ **implemented, pending audit/cut**
...
- **Motivation**:agent 在 debugging 高频出现"哪种 error 最多"/ "哪 tag 在 crash
  ...
  两项一起省。
- **Resolution**:`search_logs` 原 tool 扩参,默认行为不变:
  - `count?: true`(literal true;count mode 不返 record body)
  - `groupBy?: "tag" | "level" | "pid"`
  - `top?: number`(1..100,配 `count:true`)
  - 输出:`{entries:[], scanned, matched, groupBy, counts:[{group,count}], groupsTotal, otherCount}`
- **设计要点**:与 Block A narrowing-filter 共存(必须仍带 query / level / 时间范围 /
  mark / tags);`count:true` 不能变成 fetch-all-by-summary;aggregation mode 不接受
  `cursor`;`errorType` 未做,因为当前没有 stable parser/classifier。
- **未决问题**:
  - groupBy 字段集是否要支持复合 key(`errorType + tag`)
  - 跨 EvidenceSource 是否要一致 aggregation 接口(Poppo HTTP 的 `outcome` 字段
    vs logcat 的 `tag`)
  - `errorType` 是否值得在有稳定 classifier 后单独补
...
- **触发条件**:v2-A 上线后实测发现 `confidence:"high"` 的 source candidate
  准确率仍 < 70%
