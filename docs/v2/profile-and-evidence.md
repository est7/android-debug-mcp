...
`search_logs` 没有 source 分发层,直接在 handler 里检查 `query / level / sinceTs / beforeMark / afterMark / tags`
至少一个(buffer 单独不算 — default `main` 已是最大池;excludeTags 单独不算 — negative;
**cursor 单独不算** — cursor codec 只 carry `{offset, scanned}` 不 bind filter,接受裸 cursor
就给了 hand-crafted bypass;约定是"分页时携带同一 filter",每页都过同一个闸门)。
