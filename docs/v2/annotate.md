...
- Element subset filter(`annotationIds?: number[]`)—— 如真 case 出现
  再加,v1 全画
...
### 2026-05-28 · v2-F.2 annotate iteration(三 sub-features 单 sprint,翻案 + 设计 lock)

Locked: 2026-05-28(grill 前的 draft;codex 通过后再正式 lock)。三 sub-features
合一 sprint 单 cut(v0.5.4 候选)。

#### 触发与翻案声明

触发(per backlog § v2-F.2):
- (a) **Badge collision** —— `docs/v2/annotate.md` Q12 已显式 punt 给 v2-F.2:
  "v1 不做,留 v2-F.2 触发条件"。本 amendment 是 Q12 触发后的执行,**不算翻案**
  (lock 明文授权)。
- (b) **Text labels in badges** —— v2-F.0/F.1 lock 期是 "badge = 纯数字 ID,
  element class/text 走 mapping 不进 badge"(见 § 文档定位 / § 决策表 Q5/Q7)。
  本 amendment **翻案**:badge 加 class 短形式 ASCII label,与"7-segment digit
  only"哲学背离。
- (c) **`annotationIds?: number[]`** —— v0.5.0 lock 显式 out-of-scope。
  v0.5.2 (v2-F.3) 加 `filter` 让 server 端 prune 列表,(c) 是 "filter prune 之后
  agent 再 subset 一次画" 的更细粒度。**不算翻案**(扩展,非反向)。

(b) 的翻案 trigger:v0.5.0 ship 后 agent 反馈"光看数字记不住对应什么"(见
backlog 触发条件 #2);v2-F.3 ship 后 agent 现在能 filter 出 10-20 个目标 element,
mapping 仍要扫整个 JSON 才知道 badge 1 对应啥。class 标签让 badge 自描述。

#### F2-Q1:本 sprint 是否三件齐?

**Decision:三件齐,单 cut v0.5.4。**

(a)+(b)+(c) 都触及 annotate.ts paint 路径,合做一次 codex grill + 实施 + 真机
acceptance 比 split 三 sprint 节省 ~2× codex 轮数 + ~3× 真机 fixture cost。
v2-F.3 已证 "单 sprint 多 sub-feature with 共享 schema" 可行。

#### F2-Q2:Badge collision avoidance 算法

**Decision:5 候选位 first-non-overlap;全 overlap 退化到原 v0.5.0 行为
(outside-TL clamped to viewport)。Collision 与 paint 都用同一 `clampBadgeRect`
helper 产出的 viewport-clipped 矩形,避免"算的 rect 与画的 rect 不同步"。**

```ts
// Round 1 amendment(codex STOP 2026-05-28 #2):collision 检测必须基于实际
// 绘制的 clamped/clipped 矩形,否则部分越屏的 badge 在画布上是裁剪过的小条,
// 但 placedBadgeRects 上是越屏的大矩形 — 后续 collision 误判。新 helper:
function clampBadgeRect(
  candidateL: number, candidateT: number,
  badgeW: number, badgeH: number,
  viewport: { w: number; h: number },
): { l: number; t: number; r: number; b: number } {
  // 半开 [l, r) × [t, b) 与 element_filter.ts / hit_test.ts 一致。
  const l = Math.max(0, Math.min(viewport.w, candidateL));
  const t = Math.max(0, Math.min(viewport.h, candidateT));
  const r = Math.max(0, Math.min(viewport.w, candidateL + badgeW));
  const b = Math.max(0, Math.min(viewport.h, candidateT + badgeH));
  return { l, t, r, b };
}
```
...
For each element in paint order:
  candidates = [
    inside-TL  (l + INSET, t + INSET),
    inside-TR  (r - INSET - badgeW, t + INSET),
    inside-BL  (l + INSET, b - INSET - badgeH),
    inside-BR  (r - INSET - badgeW, b - INSET - badgeH),
    outside-TL (l, t - badgeH),                  // 不再 max(0,...) 在此处;clamp 在 helper
  ]
  for each candidate (l_raw, t_raw):
    drawn = clampBadgeRect(l_raw, t_raw, badgeW, badgeH, viewport)
    skip if drawn 完全空 (r-l===0 || b-t===0,意味着此 candidate 完全越屏)
    skip if drawn 与 placedBadgeRects 任一 overlap
    take this one, push `drawn` to placedBadgeRects, paint at drawn
    break
  if no candidate fit:
    // degraded path:用 candidates[4] (outside-TL) 的 clamped rect
    // 即便它撞了 placedBadgeRects 也接受 — overflow/clip 是显式 ack
    drawn = clampBadgeRect(candidates[4].l, candidates[4].t, badgeW, badgeH, viewport)
    push `drawn` to placedBadgeRects
    paint at drawn
```

候选顺序意图:**先保 inside 4 角**(badge 与 bbox 整齐,agent 视觉 mapping 强),
inside 全撞了才落 outside。这是 Q12 punt 原文 ("inside-tr / inside-bl /
inside-br / outside-tl / outside-tr") 的精确执行,但本 amendment 去掉 outside-TR
(实测密 UI 上 outside-TR 也很容易撞下一 element 的 outside-TL,5 候选已够)。

候选 inside 的 fits 检查复用现 `placeBadge`:`badgeW + 2*INSET <= bboxW * 0.5
&& badgeH + 2*INSET <= bboxH * 0.5`。不 fit 自动跳过那个 candidate。

`placedBadgeRects: Array<{l, t, r, b}>` 累加 **clamped/drawn** rect(不是 raw
candidate),每画一个 badge 把它推入。Overlap test = standard axis-aligned bbox
重叠(半开 `[l, r) × [t, b)`,与 hit_test.ts / element_filter.ts 一致)。

**超大 badge 显式 degrade**:若 `badgeW > viewport.w || badgeH > viewport.h`,
clamped rect 必然 partial / clip。Collision avoidance 此时是 *best-effort*,
不保证不撞;5×7 atlas + 8-char label cap 让这种情形极罕(实测 badge ~120×56,
viewport ≥ 480×800 都够)。Description 不暴露此 corner,纯实现层 invariant。

**Round 1 amendment(codex STOP 2026-05-28 #2):** 原稿 fallback 写
"outside-TL clamped to viewport",但伪码仅在 top y 做 `max(0, t - badgeH)`,
未 clamp left / right / bottom。部分越屏 element(超长 list 顶 / 底,或刚滑出
viewport 边)的 badge 会:绘制时被 painter clip 到 viewport 内的小矩形,但
`placedBadgeRects` 上是越屏的大矩形。后续 badge collision 拿大矩形比较,误判
"撞了" → 选 outside-TL fallback → 又 clip 又撞,雪上加霜。Codex 抓为
"算的 rect 与画的 rect 不同步" contract leak。最终落点:抽 `clampBadgeRect`
helper,collision + paint 都基于同一 clamped 输出;skip "完全空" candidate;
no-fit fallback 也用 clamped rect 推入。Phase 1 单测覆盖 partial-clip 边界。

复杂度:O(N²) worst case(N elements,每 element 检查 5 候选 × 累计 N rects)。
N ≤ 100(`limit` cap)→ 50000 比较 / call,negligible。

**不引入** badge-vs-bbox 重叠避免(只查 badge-vs-badge);**不引入** outside-TR/BR
(5 已够)。

#### F2-Q3:Label 内容

**Decision:label = `Element.class` FQCN 末段截断到 ≤ MAX_LABEL_CHARS (8)。**

例子:

```
android.widget.Button             → "Button"     (6 chars,留全)
android.widget.EditText           → "EditText"   (8 chars,留全)
android.widget.ImageButton        → "ImageBut"   (11→8,截尾)
androidx.recyclerview.widget...   → "Recycler"   (14→8,截尾)
android.view.View                 → "View"
android.widget.FrameLayout        → "FrameLay"   (11→8,截尾)
```

派生公式:`label = className.split('.').pop().slice(0, 8)`。

理由:
- FQCN 末段是 agent 最熟悉的语义片段(`Button` / `EditText`)。
- 8 chars 是 badge 视觉宽度 + 5×7 字体宽度的折中:8 × (5+1=letterW+gap) = 48 px,
  加 BADGE_PAD_X * 2 = 20 → badge 宽 ~68 px。屏幕 1080 width 上一行最多 ~15 badges,
  agent 仍能阅读。
- 不画 `Element.text`(用户文本):太长 + unicode + 隐私(用户输入可能含敏感字串)。

不加任何 "类簇" 抽象(`Btn` / `Txt`):agent 看 LLM-friendly 全词更好;速记由 LLM
自己做。

#### F2-Q4:ASCII-only vs i18n

**Decision:ASCII-only(uppercase A-Z + lowercase a-z + 0-9 + `:`),共
**63 glyph** bitmap atlas(`26 + 26 + 10 + 1`,无 space)。**

`Element.class` 是 Java FQCN,纯 ASCII;不可能出现 unicode label。Element.text
*会* 含中文 / emoji 但**本 sprint 不画 text 进 badge**(F2-Q3 lock),所以
i18n 当下无 use case。

不走 TTF runtime / build-time pre-raster(7-segment digit-only 哲学:无 font
asset,bun --compile 友好,glyphs 是 data 不是 IO)。

Future v2-F.4 候选:若 popposhell Compose 上线 + 真出现 "agent 想看中文 label" 反馈,
启动 i18n badge(可能走 TTF + build-time atlas pre-raster)。本 amendment lock
ASCII-only,future 触发不破。

#### F2-Q5:Glyph atlas 形态

**Decision(Round 1 修订):5×7 bitmap atlas as
`Readonly<Record<string, readonly number[]>>`,每个 char 一个 7-element
number array(每 element 5 bits 表示该行哪些列亮)。**
...
每 char 占 7 数字常量 = 7 × 4 字节 ≈ 28 字节;63 chars × 28 = ~1.7 KB total
data。Tree-shake 友好,bun --compile pure data。

**Round 1 amendment(codex advisory 2026-05-28 #1):** 原稿先 lock 类型为
`Readonly<Record<string, readonly bigint[]>>` 然后立刻说 "bigint 不必要,用
number"。Codex 抓为内部类型矛盾。最终落点:统一锁 `number[]` per row,与
"5 bits fits in number" 解释一致;`Number.MAX_SAFE_INTEGER` 远大于 5-bit,
JS 测试断言数值 bitmask 直读最方便。

`drawChar(img, x, y, ch, color, scale)`:scale = pixel multiplier (1 = 5×7 原大,
2 = 10×14 等);badge label 用 scale=2(10×14 char,与 24×40 digit 视觉协调)。
`measureText(s, scale)`:返 `{width, height}`,跟 `measureNumber` 同形。

数据放 `server/src/annotate/glyphs.ts` 末追加 `ASCII_GLYPHS = {...}`;不动现
`DIGIT_SEGMENTS`(数字仍走 7-segment,与 atlas 不混)。

#### F2-Q6:Badge 内 label 与 digit 的布局

**Decision:badge 内容 = `<label> + ":" + <annotationId>`,横向排列。**
...
数字仍用 7-segment renderer(粗壮、视觉重),label + `:` 用 5×7 atlas(细、信息密)。
垂直对齐:label baseline 与 digit baseline 居中对齐(label height 14 < digit height
40,label 上下各加 padding 让中心对齐)。

label 与 ":" 之间 GAP=2px;":" 与 digit 之间 GAP=BADGE_GAP=4px(沿用 v0.5.0)。
总 badge 宽 = `BADGE_PAD_X * 2 + measureText(label).width + 2 + measureText(":").width + 4 + measureNumber(id).width`(Round 1 修订:codex advisory #3,字段名 `.width` 与现 `measureNumber` `server/src/annotate/glyphs.ts:51-62` 对齐,不用 `.w`)。

**Round 3 candidate** (future spike):label 与 digit 二选一(由 element 类型决定)——
当前固定两段,简单优先。

#### F2-Q7:`annotationIds?: number[]` shape

**Decision:`z.array(z.number().int().positive()).min(1).max(100).optional()`。**

```ts
// inputSchema (capture) 增量:
annotationIds: z.array(z.number().int().positive()).min(1).max(100).optional()
```

语义:**post-filter post-truncate 子集选择器**。Caller 提供 annotationId 列表
(1-based,对应同次响应 `annotation.elements` 在 filter+truncate 后的 1..N 编号),
server 只画/只返这些 ID。

应用顺序:`raw dump → filter → truncate(limit) → annotationIds subset →
paint + emit`。If `annotationIds` 缺,subset = 全 filter+truncate 集(同 v0.5.0/v0.5.2 行为)。

**Round 1 amendment(codex STOP 2026-05-28 #1):** 原稿用字段名
`annotateElementIds`,字面 revive v2-F.1 lock 显式 anti-cache 的 `elementId`
词汇(`annotate.md:41-43` / `:106-118` 明示 `annotationId` 是 response-local,
**不是** server-side element identity)。Codex 抓为命名漂移 — agent 看名字
会错以为这是跨调用稳定 ID。最终落点:字段名重命名为 **`annotationIds`**(对应
"annotation IDs to keep"),与现 schema 已用的 `annotationId` 单数同源。F2-Q10
freshness 段(下)显式声明这是 current-call ordinal selectors,不是 durable
identity。Doc 内 26 处旧名全 replace_all 改新名;backlog/handoff/element-interaction.md
里的历史引用 **不动**(那些文档自身已 lock,保留 "annotateElementIds" 作为
v2-F.2 重命名前的概念名,跨文件历史可追)。

#### F2-Q7b:annotationIds **stale-ordinal freshness 模型**(Round 1 新增)

**Decision:`annotationIds` 是**当前调用** post-filter+truncate 集的 1-based 序数
选择器,**不是** 跨调用稳定 element identity。**

含义:
- Agent capture call N 看到 `annotation.elements[*].annotationId = 1..20`,
  选 `[3, 7, 12]` 想 focus —— 这 3 个 ordinal **只对 call N 的 filter+truncate
  集合有效**。
- Call N+1 即使 caller 把同 `filter` + 同 `limit` + 同 `annotationIds: [3,7,12]`
  发上来,**ordinal 3/7/12 对应的 element 实际可能不同**:UI 已变(scroll /
  rerender / 新弹窗 / 系统 dialog 进退),uiautomator 重 dump 出 element 的
  z-order / 父子顺序不保证完全一致。
- Server **只**校验 ordinal in range `[1, filteredCount-after-truncate]`;不做
  跨调用 stale 检测(没有 stable identity 可比)。
- **Agent 责任**:每次 call 后 verify `annotation.elements[i]` 的 `bounds` /
  `text` / `class` 是否仍是上次看到的目标;若怀疑 UI 已变,**不要**直接 reuse
  ordinals,先 fresh capture 看新编号。

Description 必须 explicit 这条契约 —— 不写就是 footgun:agent 缓存 ordinals →
UI 一变就脱靶。F2-Q11 description 增量段加 stale-ordinal 段。

理由:这正是 v2-F.0/F.1 反复 lock 的 "no cross-call element identity" 设计
立场。`annotationIds` 在 v2-F.2 必须遵守同一边界 —— 它是 *response-local* 工具,
不是 *cross-call* selector。

#### F2-Q8:`annotationIds` 与 `annotateElements:true` 的边界

**Decision:`annotationIds` 仅在 `annotateElements:true` 才允许;
否则 `query_malformed`(同 v2-F.3 的 filter/limit 边界体例)。**

Handler-side guard:`!wantsAnnotate && annotationIds !== undefined → throw`。
合并到现有 reject 条件(v0.5.3:filter/limit/annotationIds 任一在 non-annotate
路径)。

#### F2-Q9:out-of-range / dedup / order 语义

**Decision:**

- 任一 id ≤ 0 或 id > filteredCount-after-truncate → `query_malformed`,
  message 明示 "annotationId N out of range; current filter+truncate yields M
  elements"。
- 重复 id(`[1,1,2]`)→ dedupe 静默,服务端按 unique 处理(不 reject;agent 误
  传不致命)。
- order 不重要:`annotation.elements` 永远以 annotationId ascending 排序输出
  (不按 caller 输入顺序);badge paint 顺序同 ascending(deterministic)。
- length cap 100 与 limit 边界一致。

#### F2-Q10:`annotation.elements` schema 与 invariants

**Decision:不动 schema。`annotation.elements[i].annotationId` 仍是 filter+truncate
集内的原 1-based 编号(**不**重新编号成 1..len(subset))。**

例子:filter+truncate → 20 elements(IDs 1..20)。Caller `annotationIds:
[3, 7, 12]`。Response:`annotation.elements = [{annotationId:3,...}, {annotationId:7,...},
{annotationId:12,...}]`,**不**是 1..3。Badge 上画的也是 "3" / "7" / "12"。

Invariants(已 lock 的 v0.5.0 4 条 + v0.5.2 1 条 + 本 amendment):
- `annotation.elementCount === annotation.elements.length`(v0.5.0)
- 当 `error !== null` → `elements.length === 0 && elementCount === 0`(v0.5.0)
- `filteredCount >= elementCount`(v0.5.2)
- 当 `annotationIds` 缺:`elements` 长度 === min(filteredCount, limit)
- 当 `annotationIds` 在:`elements` 长度 === |unique(annotationIds) ∩ [1..filteredCount-after-truncate]|
  (server 已 reject out-of-range,所以等价 = unique(annotationIds).length)

annotationId 的"原编号"语义让 agent 能在**同次响应内**跨 filter / annotate /
subset 三 sub-feature 引用同一 element 标号(badge 上画 "3" + `annotation.elements`
里也是 `{annotationId:3,...}`,agent 看图调坐标走 `annotation.elements[i].center`
即用)。若 server 重新编号 1..len(subset),subset 模式下 badge "1" 与全集 badge "3"
冲突,agent 视觉脑链断。

**Round 1 amendment(codex STOP 2026-05-28 #1):** F2-Q10 原稿用 "跨 sub-feature
引用同一 element" 措辞 — codex 警觉这暗示 "跨调用"。重写为 "**同次响应内** 跨
sub-feature(filter / annotate / subset)引用同一 element 标号"。跨调用的 stale
ordinal 见 F2-Q7b;F2-Q10 只锁单次响应内一致性。

#### F2-Q11:输出字段 + audit 增量

**Decision(本 amendment 加):**

```ts
// outputSchema annotation 增量:
{
  ...existing fields (screenshotPath, elementCount, error, elements,
                       unfilteredCount, filteredCount, truncated?, warnings?),
  subsetRequested: z.number().int().nonnegative().optional(),  // |annotationIds| 原长(dedup 前)
  subsetApplied: z.number().int().nonnegative().optional(),    // unique 后真用的长度
}
// subsetRequested / subsetApplied 仅在 annotationIds 在时出现;缺时两都缺。
```

audit(commands.jsonl + capture events.jsonl):同 spread `annotateAuditFields` 加
`annotationIds?: number[]` + `subsetApplied?: number`(不 spread `subsetRequested`
to keep audit row 紧凑;`annotationIds` 原数组本身就 self-describe 了)。

#### F2-Q12:collision avoidance 与 annotationIds 的交互

**Decision:collision 检测只在 subset 内做。**

逻辑:annotationIds 应用后,paint 只迭代 subset elements;collision
`placedBadgeRects` 累加器也只看 subset。

理由:**未画的 badge 不存在视觉冲突**。Agent 显式让我们 subset 后,正是因为想
declutter — collision 算法不该把"假想的未画 badge"算进竞争。

#### 实施分期

- **Phase 1**:三 sub-feature 同步落地 + 单元 / 集成测。
  - `server/src/annotate/glyphs.ts` 加 `ASCII_GLYPHS` atlas + `measureText` + scaled metrics
  - `server/src/annotate/paint.ts` 加 `drawChar` / `drawText` primitive
  - `server/src/annotate/annotate.ts`:
    - `placeBadge` → `placeBadgeWithCollision(elementRect, badgeSize, placedRects, viewport)` —
      5 候选 first-non-overlap
    - `paint` loop 维护 `placedBadgeRects: Rect[]`
    - badge 内容从 "id only" → "label:id";引入 label 派生 `classShortLabel(class)`
  - `server/src/mcp/tools/capture.ts`:
    - inputSchema 加 `annotationIds?: number[]`
    - handler 在 filter+truncate 之后、paint 之前应用 subset + out-of-range reject
    - outputSchema annotation 增量字段
    - audit row 增 `annotationIds?` + `subsetApplied?`
    - description 同步加 annotationIds 用法 + reject 触发
  - Tests:
    - glyphs unit:ASCII atlas pixel-correctness(spot-check A / Z / 0 / 9 / ":")
    - measureText unit(scale=1/2 边界)
    - annotate unit:5-候选 fallback(密 UI fixture,3+ inside-TL 撞)
    - annotate unit:label 渲染(字符串"Button:1" measureText 总宽 = 公式预期)
    - capture 集成:annotationIds 端到端(subset reject + subset dedup +
      annotation.elements 顺序 ascending + subsetRequested/Applied 字段)
    - capture 集成:annotationIds without annotateElements:true → query_malformed
- **Phase 2**:codex post-impl audit + 真机 acceptance(Poppo 多 element 屏跑
  badge collision + label + subset 三 sub-feature)+ cut **v0.5.4**。

#### 显式 out-of-scope(本 sprint 不做)

- TTF / pre-raster atlas / i18n 中文 badge label(future v2-F.4 触发条件)
- Element.text 作 badge label(隐私 / 长度 / unicode 三重 NO)
- badge-vs-bbox 重叠避免(只 badge-vs-badge)
- collision-aware re-route to outside-TR / outside-BR(5 候选已够)
- label 类簇缩写(`Btn` 替 `Button`)— agent 自做
- subset 取号自由(`annotationIds:[100,200,300]` 这种 ID 跨 limit 必报错)
