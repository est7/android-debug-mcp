...
| Q8 | result 形态 + 顺序 | 返回 `{elements: Element[], capturedAt, captureId, elementCount, windowCount}`;`elements` 按 **z-order 倒序**(顶层 window 在前 —— 文档序最后一个 root 先出,v2-A `hit_test` 已锁文档序最后 = z-order 最顶);同一 window 内 DFS 后序(子节点先于父节点) |
