...
2026-05-22 Phase 2 实施:open decision #3 定形(`resolveProjectRoot` 既接受
`start_session` 显式入参、又回落 cwd,统一经 git-toplevel 归一);§ 2.0 描述据此更正。
2026-05-22 Phase 4 实施:open decision #4 定形(`map` 的 `commands.jsonl` 记 `rg`
命令字面量,按默认);`SourceError` 经 `wrapToolHandler` 渲染,与 `AdbError` 同构。
...
- `server/src/store/paths.ts` — `resolveProjectRoot({projectRoot?,cwd?})`:探测
  目录 = 显式 `projectRoot`(若有)否则 `cwd`,统一经 `git rev-parse
  --show-toplevel` 归一(子目录归一到仓库根);非 git → `null`。按探测目录
  memoize,`resetPathsCache()` 一并清。详见 open decision #3。
...
1. **Phase 0 ✓ 已定形** —— 多 root XML 的"最顶层窗口":取文档序最后一个、且
   包含 `(x,y)` 的 root(见 § Phase 0 hit-test tie-break 规则 2)。
2. **Phase 2 ✓ 已定形**(commit `d0ebcaa`)—— Poppo 是 **ViewBinding-only**:无
   `findViewById`、无 kotlin synthetics、DataBinding 声明未用。recipe pattern 集
   与 `SourceCandidate.kind` 见 § 2.1 / § 2.2。
3. **Phase 2.0 ✓ 已定形** —— `resolveProjectRoot` **既接受 `start_session` 显式
   入参、又回落 cwd**:探测目录 = 显式 `projectRoot` 否则 `cwd`,统一经 `git
   rev-parse --show-toplevel` 归一,非 git → `null`。原拟"仅 git-toplevel" 会落
   错根 —— MCP server 的 cwd ≠ 被调试 app(Poppo 在 `submodulepoppo/`),持久化
   会落错根、Phase 4 `map` 会 `rg` 错仓库。本结论与 design-lock § C「源码根只认
   显式 `projectRoot`」一致,未翻案 design lock(无需 amendment)。
4. **Phase 4 ✓ 已定形** —— `map` 的 `commands.jsonl` 记 `rg` 命令字面量(按
   默认):每次 `rg` 调用写一条 `{tool:"map_ui_node_to_source", rg:"rg <args>"}`,
   对标 v1 `commands.jsonl` 记录外部命令的语义。
